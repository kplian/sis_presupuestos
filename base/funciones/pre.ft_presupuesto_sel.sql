...
if(p_transaccion='PRE_PRE_SEL')then
    	...
    	begin
           ...
           ELSE
             v_join_ewf = 'INNER';
           END IF;
		   v_aux= 'ewf.obs::varchar as obs_wf,';
			v_valor='join wf.testado_wf ewf on ewf.id_estado_wf = pre.id_estado_wf ';
           ...
           IF v_parametros.tipo_interfaz = 'PresupuestoFor' THEN
            ...
            IF v_parametros.tipo_interfaz = 'PresupuestoVb' THEN
                 IF p_administrador !=1 THEN
                      v_filadd = ' (ewf.id_funcionario='||v_parametros.id_funcionario_usu::varchar||' ) and  (lower(pre.estado) in (''revision'')) and ';
