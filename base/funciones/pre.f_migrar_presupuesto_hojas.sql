...
CREATE OR REPLACE FUNCTION pre.f_migrar_presupuesto_hojas (
)
RETURNS varchar AS
$body$
DECLARE
...
BEGIN
  ...
  for v_registros in (
        select '2018'::integer         as gestion,
        t.codigo as codigo_presupuesto,
        case when t.mov_pres='{ingreso}' then 1104 else 1103 end as id_partida, t.mov_pres,
        case when t.mov_pres='{ingreso}' then 1 when t.mov_pres='{egreso}' then 2 else 3 end as tipo_pres,
        t.descripcion,
        0 as m1,
        0 as m2,
        0 as m3,
        ...
        where t.movimiento='si'
        and t.id_tipo_cc not in (select id_tipo_cc from param.tcentro_costo)
        and not t.codigo ilike 'X_%'
        )
  LOOP
