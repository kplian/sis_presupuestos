...
AS
  SELECT pareje.id_partida_ejecucion,
         ...
         pareje.cantidad_descripcion,
         cc.id_gestion,
         pareje.tipo_ajuste_formulacion --#41
  FROM pre.tpartida_ejecucion pareje
       JOIN pre.tpresupuesto pre ON pre.id_presupuesto = pareje.id_presupuesto
       JOIN param.tcentro_costo cc ON cc.id_centro_costo = pre.id_presupuesto
       JOIN param.ttipo_cc tcc ON tcc.id_tipo_cc = cc.id_tipo_cc
...
/**********************************I-DEP-JJA-PRE-02-06/08/2020****************************************/ 
select pxp.f_insert_testructura_gui ('PART', 'REPPRE'); --#46
/**********************************F-DEP-JJA-PRE-02-06/08/2020****************************************/

/**********************************I-DEP-JJA-PRE-0-27/08/2020****************************************/ 
select pxp.f_insert_testructura_gui ('ANAIMP', 'REPPRE'); ----#PRES-5 
/**********************************F-DEP-JJA-PRE-0-27/08/2020****************************************/

/**********************************I-DEP-JJA-PRE-0-28/09/2020****************************************/ 
  select pxp.f_insert_testructura_gui ('FORPRES', 'REPPRE'); --#PRES-6
/**********************************F-DEP-JJA-PRE-0-28/09/2020****************************************/
...
SELECT cec.id_centro_costo,
       ...
       pre.id_proceso_wf,
       tp.movimiento AS movimiento_tipo_pres,
       ((('('::text || tp.codigo::text) || ') '::text) || tp.nombre::text)::character varying AS desc_tipo_presupuesto,
       tp.sw_oficial,
       pre.sw_consolidado,
       pre.id_categoria_prog,
       CASE
           WHEN cec.id_tipo_cc IS NOT NULL THEN ((('('::text || cec.codigo_tcc::text) || ') '::text) || cec.descripcion_tcc::text)::character varying
           ELSE pre.descripcion
           END AS descripcion,
       cec.operativo,
       cec.id_tipo_cc
FROM param.vcentro_costo cec
         ...
         JOIN orga.tuo uo ON uo.id_uo = cec.id_uo
         JOIN pre.tpresupuesto pre ON pre.id_centro_costo = cec.id_centro_costo
         LEFT JOIN pre.tcategoria_programatica cp ON cp.id_categoria_programatica = pre.id_categoria_prog
         JOIN pre.ttipo_presupuesto tp ON tp.codigo::text = pre.tipo_pres::text;
